...
if highlevel == "pKi": ylabel = "p$K_i$"
elif highlevel == "pKd": ylabel = "p$K_d$"
elif highlevel =="pIC50": ylabel = "p$IC_{50}$"
else: raise ValueError(f"Not implemented: {highlevel}")

l = 2048

mode = "delta"
lowlevel = "XGB" # delta_LinF9_XGB docking
mol_data = pd.read_csv(f"{target}/{target}_data_3d_{mode}_{highlevel}.csv")
vmin, vmax = 4, 9
...
b2 = axis[1,0].scatter(pca_crds[:,0][top10],pca_crds[:,1][top10],s=4,c="orange",alpha=0.8,label="top-10% docking",zorder=2)
b3 = axis[1,0].scatter(pca_crds[:,0],pca_crds[:,1],s=8,c=docking_values,cmap="cividis",alpha=0.8,vmin=4,vmax=8)
