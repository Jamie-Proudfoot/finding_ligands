...
def get_vina_filename() -> str:
    ...
    if system_name == 'Linux':
        return 'vina_linux_new' #new  #'vina_linux'
...
def correct(smi):
    """
    Correct OpenBabel protonation errors.
    This is a known error in the OpenBabel protonation model
    it affects enols, enamines, phosphates, N-O groups and tetrazoles
    by adding negative charge to O or N atoms, giving incorrect valence.
    See GitHub issues #2397 and #162 on OpenBabel
    """
    regexes = ["\w\[O\-\]\S","\S\[O\-\]\w","\[O\-\]1C","C1\[O\-\]"
               "\w\[N\-\]\=\S","\S\[N\-\]\=\w","\w\=\[N\-\]\S","\S\=\[N\-\]\w",]
    whitelist = ["1[O-])","2[O-])"]
    for regex in regexes:
        p = re.compile(regex)
        for m in re.finditer(p,smi):
            match = m.group()
            if match not in whitelist:
                smi = smi.replace(match,match.replace("-",""))
    # Correct valence violations due to H added to [X] in square brackets
    smi = smi.replace("[OH2-]","[O-]")
    smi = smi.replace("[OH2]","[OH]")
    smi = smi.replace("[NH2-]","[NH2]")
    smi = smi.replace("[CH3]","[C]")
...
def protonate_mol(mol: Chem.Mol, pH: float) -> Chem.Mol:
    ...
    if protonated_smiles != smiles: # only correct if changes made
        protonated_smiles = correct(protonated_smiles) #new
